...
Servo::Servo(PWMDriver *pwmDriver, int outputNum, float plusMinusRange):
   ...
   this->physMin = -plusMinusRange;
   this->physMax = plusMinusRange;
   this->physStart = 0;
   this->physPos = 0;
...
int Servo::setAngle(float angle){
   if(angle > physMax) angle = physMax;
   else if(angle < physMin) angle = physMin;
   this->physPos = angle;
...
/**
 * Set servo angle relative to start position
 */
int Servo::setAngleRelative(float angle){
   float angle_relative = angle + physStart;
   return setAngle(angle_relative);
}

int Servo::setMountingRange(float physMin, float physMax, float physStart)
{
   if(physMin >= physMax || physStart < physMin || physStart > physMax) {
      perror("Servo: Error, mounting range is illegal.");
      return -1;
   }
   this->physMin = physMin;
   this->physMax = physMax;
   this->physStart = physStart;
   setAngle(physStart);
   return 0;
}
